...
class Test:
    ...
    def test_position_outputs(self, position: str = chess.STARTING_FEN, n: int = 50):
        ...
        for action in sorted(agent.mcts.root.edges,
                             key=lambda edge: (edge.W / (edge.N if edge.N != 0 else 1)) + edge.upper_confidence_bound(),
                             reverse=True):
            print(action)
...
def test_predict_vs_predict_batch():
    ...
    nn: Model = tf.keras.models.load_model("models/model-2024-11-24_10-44-19.keras")
    # warm up
