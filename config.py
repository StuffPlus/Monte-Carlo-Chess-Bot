...
# boolean values: side to move, castling rights for every side and every player, is repetition
amount_of_input_planes = (2 * 6 + 1) + (1 + 4 + 1)
INPUT_SHAPE = (n, n, amount_of_input_planes)

# ============= NEURAL NETWORK OUTPUTS =============
# the model has 2 outputs: policy and value
# output_shape[0] should be the number of possible moves
...
# amount of hidden residual layers According to the AlphaGo Zero paper: "For the larger run (40 block, 40 days),
# MCTS search parameters were re-optimised using the neural network trained in the smaller run (20 block,
# 3 days)." ==> First train a small NN, then optimize longer with a larger NN.
