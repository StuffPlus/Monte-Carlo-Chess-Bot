...
class Game:
    ...
    def play_one_game(self, stochastic: bool = True) -> int:
        ...
        while not self.env.board.is_game_over():
            ...
            if counter > config.MAX_GAME_MOVES or self.env.board.is_repetition(3):
                ...
                logging.info(f"Game over by move limit ({config.MAX_GAME_MOVES}) or by repetition. Result: {winner} ")
                full_game = False
                break
        if full_game:
    ...
    def play_move(self, stochastic: bool = True, previous_moves: tuple[Edge, Edge] = (None, None),
                  save_moves=True) -> None:
        """
    ...
    def create_puzzle_set(filename: str, type: str = "mateIn2") -> pd.DataFrame:
        ...
        # set column names
        puzzles.columns = ["fen", "moves", "rating", "type", "GameURL"]
